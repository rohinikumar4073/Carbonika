                    ...
                    if (questions.hasOwnProperty(i)) {
                        ...
                        for (j in answers) {
                            if (answers.hasOwnProperty(j)) {
                                answer   = answers[j],
                                optionId = inputName + '_' + j.toString();
                                ...
                                var optionLabel = '<label for="' + optionId + '">' + answer.option + '</label>';
								  ...
								  var answerContent="";
								 if(inputName=="question0"){
									 if(j==0)
									 samphtml='<img src="img/veg.png" height=50 width=100 style="display:inline"/>';
									 else if(j==1)
						             samphtml='<img src="img/veget.png" height=50 width=100 style="display:inline"/>';
									  else if(j==2)
						             samphtml='<img src="img/nonveg.png" height=50 width=100 style="display:inline"/>';

                               answerContent = $('<li></li>')
                                    .append(input).append(samphtml)
                                    .append(optionLabel);
									}else{
										answerContent = $('<li></li>')
                                    .append(input)
...
trueAnswers.push($('<div />').html(answer.option).text());
                ...
                var correctResponse = plugin.method.compareAnswers(trueAnswers, selectedAnswers, selectAny,questionIndex);
            ...
            nextQuestion: function(nextButton) {
                var currentQuestion = $($(nextButton).parents(_question)[0]),
					...
					$(".tips").html(quizJSON.questions[$(currentQuestion).index()].tip);
            ...
            completeQuiz: function(tempval) {
                var levels    = [
                                    quizJSON.info.level1, // 80-100%
                                    quizJSON.info.level2, // 60-79%
                                    quizJSON.info.level3, // 40-59%
                                    quizJSON.info.level4, // 20-39%
                                    quizJSON.info.level5  // 0-19%
                                ],
                    score     = parseInt($(".progress-label").html());
                    levelRank = plugin.method.calculateLevel(score);
                    levelText = $.isNumeric(levelRank) ? levels[levelRank] : '';
                    scr=100-score;
                $(_quizScore + ' span').html(scr + ' / ' + 100);
            ...
            compareAnswers: function(trueAnswers, selectedAnswers, selectAny,questionIndex) {
				$(trueAnswers).each(function(index, element) {
                    if(element==selectedAnswers[0]){
						
                         var temp=(quizJSON.questions[questionIndex].a[index].corbonprint);
						 var tempval=parseInt($(".progress-label").html())+parseInt(temp);
						 if(tempval==0 || tempval==100){
							   plugin.method.completeQuiz(tempval);
							 }
						 $( "#progressbar" ).progressbar( "value", tempval );
						}
            ...
            calculateLevel: function(correctAnswers) {
                var percent = (correctAnswers / (100.0 )).toFixed(2),
                    level   = null;
                ...
                if (plugin.method.inRange(0, 0.20, percent)) {
                    ...
                    level = 3;
                } else if (p3lugin.method.inRange(0.81, 1.00, percent)) {
                    level = 4;
                }
